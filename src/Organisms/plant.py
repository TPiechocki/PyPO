...
class Plant(Organism):
    def __init__(self, x, y, wrld):
        ...
        self._scatter_possibility = 10 # per cents
        self._initiative = 0

    def action(self):
        field = self._world.getField(self._x, self._y)
        field = field.getNeighbour(self._direction.randomDirection())
        ...
        if field is not None and field.isEmpty() and random.randint(0,100) < self._scatter_possibility:
            ...
            self._world.addNotification(repr(self) + ": Roślina się rozprzestrzeniłą.")
